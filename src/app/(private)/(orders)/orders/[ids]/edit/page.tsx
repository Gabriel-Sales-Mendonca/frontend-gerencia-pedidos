...
export default function CreateOrderPage() {
    ...
    return (
        <div className="max-w-xl mx-auto mt-10 p-6 bg-white dark:bg-neutral-600 text-neutral-900 dark:text-neutral-100 rounded-2xl shadow-md space-y-6">
            <h1 className="text-3xl font-bold mb-6 text-center">Editar Pedido</h1>
                    ...
                    {formData.products.map((product, index) => (
                        <div key={index} className="flex space-x-2 mb-2">
                            <input
                                ...
                                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                                required
                                disabled={products.includes(product.id)}
                                placeholder={`Código do Produto ${index + 1}`}
                            />
                            {formData.products.length > 1 && (
                                <button
                                    type="button"
                                    onClick={() => {products.includes(product.id) ? handleDeleteClick(serviceOrderMap.get(product.id), index) : removeProductField(index)}}
